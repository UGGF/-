...
@interface BindingNewNumController ()<UITextFieldDelegate>
{
    UIView *_bindView;
    NSTimer *_timer;
    UITextField *_check;
...
@implementation BindingNewNumController
...
- (BOOL)checkTel:(NSString *)str{
    ...
    //正则表达式
    NSString *regex = @"^((13[0-9])|(147)|(15[^4,\\D])|(18[0,1,3-9])|(17[6-8]))\\d{8}$";
...
- (void)saveBtnClick{
    ...
    session *danli = [session getInstance];
    [ZYHTTPRequest request:PHONENUMBINDING_URL(danli.token,@"" ,_check.text) Completion:^(NSDictionary *dict) {
        NSLog(@"%@",dict);
        int stat = [[dict objectForKey:@"code"] intValue];
        if (stat == 200) {
