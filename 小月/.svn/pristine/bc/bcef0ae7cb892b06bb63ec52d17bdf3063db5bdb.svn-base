...
- (void) handlePan: (UIPanGestureRecognizer *)rec{
    ...
    if (((self.mainVC.view.frame.origin.x <= 0) && (_scalef <= 0)) || ((self.mainVC.view.frame.origin.x >= (kScreenWidth - kMainPageDistance )) && (_scalef >= 0)))
    {
        //边界值管控
        _scalef = 0;
        needMoveWithTap = NO;
    ...
    if (needMoveWithTap && (rec.view.frame.origin.x >= 0) && (rec.view.frame.origin.x <= (kScreenWidth - kMainPageDistance)))
    {
        ...
        self.leftTableview.center = CGPointMake(leftTabCenterX, kScreenHeight );
    ...
    else
    {
        ...
        else if (self.mainVC.view.frame.origin.x > (kScreenWidth - kMainPageDistance))
        {
            [self openLeftView];
            _scalef = 0;
        }
