...
-(void)createKit
{
    ...
    self.identifyingTF = [[UITextField alloc]initWithFrame:CGRectMake(10, 200, WIDTH - 135, 40)];
...
-(void)registerBtnClick
{
    ...
    [ZYHTTPRequest requestRegister:self.phoneTF.text :string2 Completion:^(NSDictionary *dict) {
        NSLog(@"%@",dict);
        NSString *str111 = [dict objectForKey:@"resultState"];
        NSLog(@"%@",str111);
        if ([str111 isEqualToString:@"error"]) {
            ALERTVIEW([dict objectForKey:@"message"]);
        }else
        {
            [ZYHTTPRequest request:LOGIN_URL(self.phoneTF.text, string2) Completion:^(NSDictionary *dict) {
                ...
                if (stat == 200) {
                    ...
                    header.userId = [[dict objectForKey:@"useInfo"] objectForKey:@"userId"];
