...
- (void)btnClick{
    ...
    [ZYHTTPRequest request:SMSSEND_URL(danli.token,_phoneTF.text) Completion:^(NSDictionary *dict) {
        NSLog(@"%@",dict);
...
- (void)saveBtnClick{
    [self.view endEditing:YES];
    NSString *str =[self md5:_passwordTF.text];
    NSString *str1 = [self md5:str];
    //-substringWithRange: //按照所给出的位置，长度，任意地从字符串中截取子串
    NSString *string2 = [str1 substringWithRange:NSMakeRange(8, 16)];
    
    session *danli = [session getInstance];
    [ZYHTTPRequest request:PHONENUMBINDING_URL(danli.token, string2,_identifyingTF.text) Completion:^(NSDictionary *dict) {
        NSLog(@"%@",dict);
        int stat = [[dict objectForKey:@"code"] intValue];
        if (stat == 200) {
