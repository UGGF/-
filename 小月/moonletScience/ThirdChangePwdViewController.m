...
- (void)createChangePwdView{
    ...
    UILabel *botLab = [[UILabel alloc]initWithFrame:CGRectMake(10, 0, WIDTH-60, 100)];
    botLab.numberOfLines = 0;
    botLab.textColor = TEXTCOLOR;
    botLab.font = [UIFont systemFontOfSize:15];
    botLab.textAlignment = NSTextAlignmentLeft;
    [cicView addSubview:botLab];
    
    NSMutableAttributedString *botString = [[NSMutableAttributedString alloc]initWithString:@"检测到您正使用QQ账号登录小月只能智能枕，再次无法修改密码。建议您绑定一个手机号，可以使用该手机号登录，并对其密码进行设置。"];
    NSRange range1 = [[botString string]rangeOfString:@"QQ"];
