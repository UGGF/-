//
//  MicroBlogBindingViewController.m
//  moonletScience
//
...
@interface MicroBlogBindingViewController ()

@end

@implementation MicroBlogBindingViewController
...
- (void)createBlog{
    ...
    imaBtn.frame = CGRectMake(WIDTH/2-60, 70+60, 120, 120);
    imaBtn.backgroundColor = TEXTTINTCOLOR;
    //    unBindBtn.alpha = 0.7;
    imaBtn.layer.cornerRadius = 60;
    imaBtn.clipsToBounds = YES;
    [imaBtn setImage:[UIImage imageNamed:@"weibo"] forState:UIControlStateNormal];
    ...
    UILabel *lab = [[UILabel alloc]initWithFrame:CGRectMake(10, 301, WIDTH/2, 20)];
    lab.text = @"绑定的新浪微博:";
    ...
    UILabel *numLab = [[UILabel alloc]initWithFrame:CGRectMake(WIDTH/2+15, 301, WIDTH/2-10, 20)];
    numLab.text = @"已绑定";
    ...
    [unBindBtn setTitle:@"解绑新浪微博" forState:UIControlStateNormal];
    [unBindBtn setTitleColor:TEXTCOLOR forState:UIControlStateNormal];
    unBindBtn.titleLabel.font = [UIFont systemFontOfSize:18];
    ...
    PromptLab.text = @"绑定微博可以保障您的账户安全，当您手机账号无法登陆时，可通过此处绑定的微博实现第三方登录进入绑定账户。";
...
- (void)unBindBtnClick{
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"警告" message:@"解绑新浪微博将对您的账户安全产生隐患\n确定解绑吗？" preferredStyle:UIAlertControllerStyleAlert];
        ...
        [ShareSDK getUserInfo:SSDKPlatformTypeSinaWeibo
               onStateChanged:^(SSDKResponseState state, SSDKUser *user, NSError *error)
         {
             if (state == SSDKResponseStateSuccess)
             {
                 ...
                 [ZYHTTPRequest request:SINAUNBINDING_URL(danli.token, user.credential.token) Completion:^(NSDictionary *dict) {
                     NSLog(@"%@",dict);
