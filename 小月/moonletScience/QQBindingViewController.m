...
//    [unBindBtn addTarget:self action:@selector(fanhui) forControlEvents:UIControlEventTouchUpInside];
    ...
    self.numLab = [[UILabel alloc]initWithFrame:CGRectMake(WIDTH/2+15, 301, WIDTH/2-10, 20)];
    self.numLab.textColor = TEXTCOLOR;
    self.numLab.text = @"已绑定";
        ...
        [ShareSDK getUserInfo:SSDKPlatformTypeQQ
               onStateChanged:^(SSDKResponseState state, SSDKUser *user, NSError *error)
         {
             if (state == SSDKResponseStateSuccess)
             {
                 ...
                 [ZYHTTPRequest request:QQUNBINDING_URL(danli.token,user.credential.token) Completion:^(NSDictionary *dict) {
                     NSLog(@"%@",dict);
