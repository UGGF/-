...
-(void)saveBtnClick
{
    ...
    [ZYHTTPRequest request:SETTING_PERSONINFO_URL(danli.token, @"com", self.textF.text) Completion:^(NSDictionary *dict) {
...
- (void)creatTextField{
    ...
    self.textF.placeholder = @"  亲，您在哪里高就?";
...
- (BOOL) textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string{
    if ([string isEqualToString:@""] && range.length > 0) {
        ...
        if (textField.text.length - range.length + string.length > 32) {
