...
@interface CloseReportViewController ()<UITableViewDataSource,UITableViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate>
{
    NSArray *_nameArray;
    NSString *filePath;
...
#pragma mark - 修改状态栏的简便方法
    ...
    closeBtn.frame = CGRectMake(WIDTH-70, 15, 60, 20);
    closeBtn.clipsToBounds = YES;
    closeBtn.adjustsImageWhenHighlighted = NO;
    [closeBtn setTitle:@"关闭报告" forState:UIControlStateNormal];
...
- (void)createContentView{
    ...
    self.CloseTableview = [[UITableView alloc]initWithFrame:CGRectMake(0, 112, WIDTH, HEIGHT-112)];
    self.CloseTableview.delegate = self;
    self.CloseTableview.dataSource = self;
    self.CloseTableview.separatorStyle = UITableViewCellSeparatorStyleNone;
    self.CloseTableview.backgroundColor = WHITECOLOR;
    self.CloseTableview.showsVerticalScrollIndicator = NO;
    [self.view addSubview:self.CloseTableview];
...
- (void)clickBtnClick:(UIButton *)sender{
    for (int i = 0; i < 100; i++) {
        if (i == sender.tag - 70) {
            _flagCloseArrar[i] = !_flagCloseArrar[i];
        }
        //        }else{
        //            _flagOpenArrar[i] = NO;
        //        }
...
- (UITableViewCell *) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    static NSString *cellID = @"CloseReportTableViewCell";
    CloseReportTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellID];
    if (cell == nil) {
        cell = [[[NSBundle mainBundle]loadNibNamed:cellID owner:self options:nil]lastObject];
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
    }
    
    cell.CloseImageBtn.backgroundColor = CAOGREENCOLOR;
    cell.CloseNameLab.text = @"得得";
    cell.CloseSignLab.text = @"发小";
    [cell.CloseSelectBtn addTarget:self action:@selector(OpenSelectBtnClick:) forControlEvents:UIControlEventTouchUpInside];
    cell.CloseSelectBtn.tag = 100 + indexPath.row;
    cell.CloseSelectBtn.selected = YES;
    return cell;
}
