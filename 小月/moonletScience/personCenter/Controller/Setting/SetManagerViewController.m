...
@interface SetManagerViewController ()<UITableViewDataSource,UITableViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate>
{
    NSArray *_nameArray;
    UIView *_PopView;
    NSString *filePath;
...
- (void)viewDidLoad {
    ...
    _nameArray = @[@"自定义管理者(无APP者)",@"驴友",@"牌友党",@"大学同学"];
    [self createNavBar];
...
- (void)createContentView{
    ...
    self.ManageTableView = [[UITableView alloc]initWithFrame:CGRectMake(0, 112, WIDTH, HEIGHT-112)];
    self.ManageTableView.delegate = self;
    self.ManageTableView.dataSource = self;
    self.ManageTableView.separatorStyle = UITableViewCellSeparatorStyleNone;
    self.ManageTableView.backgroundColor = WHITECOLOR;
    self.ManageTableView.showsVerticalScrollIndicator = NO;
...
- (void)createPopView{
    _PopView = [[UIView alloc]initWithFrame:CGRectMake(0, 152, WIDTH, 60)];
...
- (void)clickBtnClick:(UIButton *)sender{
    if (sender.tag == 70) {
        ...
        for (int i = 0; i < 100; i++) {
            if (i == sender.tag - 70) {
                _flagManageArrar[i] = !_flagManageArrar[i];
            }else{
                _flagManageArrar[i] = NO;
            }
        }
