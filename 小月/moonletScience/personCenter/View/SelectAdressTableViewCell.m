...
//    [_tickedBtn addTarget:self action:@selector(tickedBtnClick:) forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:_tickedBtn];
    
    NSArray *array = @[@"收货人",@"手机号",@"收货地址"];
    for (int i = 0; i < 3; i++) {
        _lab = [[UILabel alloc]initWithFrame:CGRectMake(40, 5+i*18, 50, 20)];
    ...
    for (int i = 0; i < 3; i++) {
        _detailLab = [[UILabel alloc]initWithFrame:CGRectMake(100, 5+i*18, 200, 20)];
        ...
        if (i == 2) {
            ...
            _detailLab = [[UILabel alloc]initWithFrame:CGRectMake(100, 5+2*18, 200, 40)];
